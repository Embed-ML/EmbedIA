    ...
    for (i=0; i<output->height; i++){
		for (j=0; j<output->width; j++){
					...
					for (l=0; l<layer.filters[0].kernel_size; l++){
						...
						if(((++cont)%tamano_del_bloque)==0){
                            ...
                            for(f=0;f<layer.n_filters;f++){
                                output->data[(f*output_hw) + i*output->width + j] += float_to_fixed(POPCOUNT(XNOR(suma,layer.filters[f].bitarray[(cont/tamano_del_bloque)-1])));
                            }
                            suma = 0;
						}
					}
            ...
            if(long_ult_bloque!=0){ //bloque extra
                for(f=0;f<layer.n_filters;f++){
                    output->data[(f*output_hw) + i*output->width + j] += fixed_add(float_to_fixed(POPCOUNT(XNOR(suma,layer.filters[f].bitarray[cont/tamano_del_bloque])) - (tamano_del_bloque-long_ult_bloque)) , layer.filters[f].bias);
                }
            }else{  //solo bias
                for(f=0;f<layer.n_filters;f++){
...
void quantdense_layer(quantdense_layer_t dense_layer, data1d_t input, data1d_t * output){
	...
	if(long_ult!=0){
        ...
        for(cont = 0;cont<dense_layer.n_neurons;cont++){
            output->data[cont] += fixed_add(float_to_fixed(POPCOUNT(XNOR(dense_layer.neurons[cont].weights[bloques_enteros],tot)) - (tamano_del_bloque-long_ult)), dense_layer.neurons[cont].bias);
        }
    }else{  //solo bias
        for(cont=0;cont<dense_layer.n_neurons;cont++){
