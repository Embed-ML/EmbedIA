 ...
 */
typedef struct {
    const float * weights;  /**< Pointer to the filter weights (kernel_size * channels) */
    float bias;             /**< Bias value for the filter */
} filter_t;
 ...
 */
typedef struct {
    const float * weights;      /**< Weights for depthwise filters (channels * kernel_h * kernel_w) */
    const float * bias;         /**< Bias values per channel */
...
typedef struct {
    ...
    float *weights;             /**< Weight matrix [input_size][output_size] */
 ...
 */
typedef struct {
    const float *sub_val;         /**< Values to subtract (e.g., mean, min, median) */
    const float *inv_div_val;     /**< Inverse of divisor (e.g., 1/std_dev, 1/(max-min)) */
...
typedef struct {
    ...
    const float *moving_inv_std_dev;        /**< Precomputed: gamma / sqrt(variance + epsilon) */
    const float *std_beta;                  /**< Precomputed: beta - moving_mean * moving_inv_std_dev */
 ...
 * issues due to the float memory allocation strategy.
 */
