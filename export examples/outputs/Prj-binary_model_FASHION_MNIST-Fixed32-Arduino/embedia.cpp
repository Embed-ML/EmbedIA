			...
			for (j=0; j<output->width; j++){
				...
				for (k=0; k<filter.kernel_size; k++){
					for (l=0; l<filter.kernel_size; l++){
						...
						if(((++cont)%binary_block_size)==0){
                            //ya tengo el num
                            output->data[(c*output_hw) + i*output->width + j] += float_to_fixed(POPCOUNT(XNOR(suma,filter.bitarray[((cont/binary_block_size)-1)+(var1)])));
                            suma = 0;
						}
					}
				}
				//termino un canal
                if(long_ult_bloque!=0){ //bloque extra
                    output->data[(c*output_hw) + i*output->width + j] += float_to_fixed(POPCOUNT(XNOR(suma,filter.bitarray[(cont/binary_block_size)+(var1)])) - (binary_block_size-long_ult_bloque));
                }
		...
		for (j=0; j<output->width; j++){
			...
			for (c=0; c<filter.channels; c++){
                ...
                if((filter.bitarray[offset_absoluto / binary_block_size] & (MSB >> (offset_absoluto % binary_block_size)))>>(binary_block_size-(offset_absoluto % binary_block_size)-1)){
                    ...
                    suma += fixed_mul((-FIX_ONE) , input.data[(c*input.height*input.width)+(i)*input.width+(j)]);
                }
			}
			output->data[delta + i*output->width + j] = fixed_add(suma , filter.bias);
		}
