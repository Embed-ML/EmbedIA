...
void batch_normalization1d_layer(batch_normalization_layer_t layer, data1d_t *data) {
    ...
    for (i = 0; i < data->length; i++) {
        data->data[i] = FIXED_MUL((data->data[i] - layer.moving_mean[i]),layer.moving_inv_std_dev[i] + layer.beta[i]);
    }
}
...
void batch_normalization3d_layer(batch_normalization_layer_t layer, data3d_t *data) {
    ...
    for (i = 0; i < data->channels; i++, ilen += length) {
        for (j = 0; j < length; j++) {
            data->data[ilen+j] = FIXED_MUL((data->data[ilen+j] - layer.moving_mean[i]),layer.moving_inv_std_dev[i] + layer.beta[i]);
        }
    }
}
