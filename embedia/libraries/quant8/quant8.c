...
void quantize_param(float* values, int size, qparam_t* qp) {
    ...
    // Encontrar el valor mínimo y máximo en el arreglo de valores
    ...
    qp->sc = (max_val - min_val) / Q_MAX;
    float zero_p = -min_val / qp->sc; // Punto cero para mapear al rango

    if (zero_p < 0) {
        qp->zp = 0;
      } else if (zero_p > Q_MAX) {
        qp->zp = Q_MAX;
      } else {
        qp->zp = (uint8_t)( round(zero_p) );
      }
}
...
void quantize_vec(float values[], quant8 qvalues[], int size, qparam_t qp) {
    for (int i = 0; i < size; ++i) {
        ...
        if (quantized_float > Q_MAX) {
            qvalues[i] = Q_MAX;
        } else if (quantized_float < 0) {
            qvalues[i] = 0;
        } else {
            // Convertir el valor cuantizado a int8_t
            qvalues[i] = (quant8)quantized_float;
        }
    }
}
...
void dequantize_vec(quant8 qvalues[], float values[], int size, qparam_t qp) {
    for (int i = 0; i < size; ++i) {
        ...
        // Almacenar el valor descuantizado en el arreglo de valores flotantes
        values[i] = dequantized_float;
    }
}
...
float mul_add_vec(quant8 a[], qparam_t qa, quant8 b[], qparam_t qb, int size){
    ...
    uint32_t i;
    for (i=0; i<size; i++){
            result+= ( (int32_t)a[i]-qa.zp ) * ( (int32_t)b[i]-qb.zp );
    }
    return result * qa.sc * qb.sc;
}
