 ...
 *Specifies the number of channels (uint16_t channels), their size (uint16_t kernel_size), the weights (xBITS *bitarray;) and the bias (half bias).
 */
 ...
 * Specifies the weights of the neuron as a vector (xBITS  * weights) and the bias (half bias).
 */
...
/*
 * Structure that stores an array of half data (half * data) in vector form.
 ...
 * Structure that stores the weights of a filter.
 Specifies the number of channels (uint16_t channels), their size (uint16_t kernel_size), the weights (half * weights) and the bias (half bias),
 * the weights (half * weights) and the bias (half bias).
 */
typedef struct{
    uint16_t channels;
    uint16_t kernel_size;
 ...
 * Specifies the weights of the neuron as a vector (half * weights) and the bias (half bias).
 */
typedef struct{
    const half  * weights;
    half  bias;
...
typedef struct {
    uint32_t length;
    const half *moving_inv_std_dev; // = gamma / sqrt(moving_variance + epsilon)
    const half *std_beta;           // = beta - moving_mean * moving_inv_std_dev 
...
void leakyrelu_activation(half *data, uint32_t length, half alpha);
...
void batch_normalization_layer(batch_normalization_layer_t norm, uint32_t length, half *data);
