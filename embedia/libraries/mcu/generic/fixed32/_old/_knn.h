 ...
 * Uses fixed-point arithmetic and function pointer for flexible distance metric selection.
 */
typedef struct
{
    ...
    fixed *neighbors_features;      /**< Training data [n_samples × n_features] in fixed-point */
    uint16_t *neighbors_id;         /**< Class labels (classifier) or target values (regressor) */
    dfixed (*distance_fn)(fixed *, fixed*, int);  /**< Distance function pointer */
