...
/**
 * @typedef dfixed
 * @brief 64-bit integer type for double-precision fixed-point operations.
 */
...
#define FIX_EXP_MAX FL2FX_CONST(9.7132951013616)
...
#define FIX_INV_2PI FL2FX_CONST(2/(2*FIX_PI)) // 2/(2π)
 ...
 * Supports input range [-8, 8), covering typical activations including ReLU6.
 * sum(Q16) * recip(Q12) = Q28, fits in int32 without overflow.
 */
#define FIX_AVG_PREC  (12)
...
#define FL2FX_CONST(F) \
    ((fixed)((int32_t)((F) * ((int32_t)1 << FIX_FRC_SZ) + 0.5)))
...
#define DFIX_ZERO (FX2DFX(FIX_ZERO))
 ...
 * @param acc Accumulator (int64_t).
 * @param w First operand (fixed).
 * @param x Second operand (fixed).
 *
 * Uses 64-bit accumulator to prevent overflow in long accumulations
 * (e.g., deep convolutions). Maintains same fractional scale as DFIXED_MAC.
 */
#define DDFIXED_MAC(acc,w,x) \
    ((acc) += ((int64_t)(w) * (int64_t)(x)))
 ...
 * - FIX_DFRC_SZ (16) is optimal for most neural network operations
 *
 * @{
 */
 ...
 * @param PREC Fractional bits for reciprocal (0-24)
...
#define FIXED_AVG_RECIP(COUNT)         FIXED_RECIP(COUNT, FIX_AVG_PREC)
 ...
 * @param base Logarithm base (positive and ≠ 1).
 * @return log_b(x).
 */
