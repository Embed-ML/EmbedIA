/**
 * @brief Computes class votes from sparse SVM support vectors using a kernel function (fixed-point implementation).
 *
 ...
 * function result. Uses high-precision fixed-point arithmetic (dfixed) for accumulation
 * with final saturation rounding to avoid overflow.
 *
 ...
 * @param[in] input Pointer to the input feature vector of size n_features (fixed-point).
 * @param[out] votes Pointer to output votes array of size n_classes (fixed-point).
 ...
 *
 * @note Accumulation uses dfixed (double precision fixed-point) for intermediate calculations
 *       to maintain accuracy before final conversion back to standard fixed-point format.
 */
    ...
    compute_t (*kernel_fn)(const svm_classifier_layer_t*, const compute_t*, const compute_t*))
{
    ...
    for (uint16_t i = 0; i < n_classes; i++) {
        for (uint16_t j = i + 1; j < n_classes; j++) {
            ...
            /* Mejora crítica: acumulación en dfixed */
            dfixed decision = FX2DFX(model->icepts[pair_idx]);

            const svm_pair_sparse_t *pair = &model->pairs[pair_idx];

            for (uint16_t k = 0; k < pair->count; k++) {
                const compute_t *sv =
                    model->vectors + (pair->data[k].idx * model->n_features);

                compute_t k_val = kernel_fn(model, input, sv);

                /* MAC en alta precisión */
                DFIXED_MAC(decision, pair->data[k].coef, k_val);
            }

            /* Conversión final con saturación */
            compute_t decision_fx = DFX2FX_RND_SAT(decision);

            if (decision_fx > FIX_ZERO)
