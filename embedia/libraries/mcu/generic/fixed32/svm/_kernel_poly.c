/**
 * @brief Computes the polynomial kernel between an input vector and a support vector (fixed-point).
 *
 ...
 *
 * @return The computed polynomial kernel value between the input and support vector (fixed-point).
 *
 * @note For degree values of 2 or 3, direct multiplication is used instead of fixed_pow()
 *       for improved performance. Dot product is accumulated in dfixed for high precision.
 */
    ...
    const compute_t              *sv)
{
    ...
    compute_t term =
        FIXED_ADD(FIXED_MUL(model->kernel.gamma, dot),
