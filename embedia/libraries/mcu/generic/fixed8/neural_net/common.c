 ...
 * ----------------------------------------------------------------------- */
/* @embedia-note
 * DOUBLE-BUFFER MEMORY ALLOCATOR:
 * - Two buffers alternate: buffer[0] grows from left, buffer[1] from right
 * - Each allocation invalidates previous allocation on same side (no fragmentation)
 * - 4-byte alignment required for Cortex-M0 and optimal for M3/M4
 * - ALLOC_BUFFER_SZ computed by Python estimator based on model requirements
 * - Collision detection: ensures buffers don't overlap in middle of pool
 * - Critical for embedded: deterministic O(1) allocation with zero fragmentation
 */
