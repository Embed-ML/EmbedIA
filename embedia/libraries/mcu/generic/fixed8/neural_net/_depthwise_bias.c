 ...
 * - Processes each channel independently (no cross-channel mixing)
 * - Handles SAME/VALID padding: compute_padding calculates pad_h/pad_w dynamically
 * - Bounds checking: skips kernel positions that fall outside valid input region
 * - Uses FIXED_MUL for 8-bit multiplication (returns dfixed/16-bit intermediate)
 * - Bias added directly without conversion (assumes pre-scaled bias values)
 * - Output indexing: channel-major order [c][i][j]
 */
