...
void multi_stft_layer(spectrogram_layer_t config, data2d_t input, data3d_t *output) {
    ...
    */
    ...
    for (c = 0; c < input.height; c++) {
        ...
        const unsigned int midpoint = config.frame_length >> 1;

        for (i = 0; i < config.n_frames; i++) {
            const unsigned int start = i * config.hop_length;

            // Aplicar ventana y zero-padding
            for (j = 0; j < config.frame_length + aux_n_fft; j++) {
                if (start + j < input.width) {
                    fixed win_value;
                    if (j < midpoint) {
                        win_value = config.window[j];
                    } else {
                        win_value = config.window[config.frame_length - 1 - j];
                    }
                    // Multiplicación: Q16.16 * Q16.16 → Q32.32 → Q16.16
                    dfixed temp = (dfixed)channel_data[start + j] * (dfixed)win_value;
                    data_re[j] = (fixed)(temp >> FIX_FRC_SZ);
                } else {
                    data_re[j] = FIX_ZERO;
                }
                data_im[j] = FIX_ZERO;
            ...
            for (j = 0; j < config.n_fft_table; j++) {
                ...
                fixed sum_sq = (fixed)(re_sq + im_sq);  // SIN 2do shift
