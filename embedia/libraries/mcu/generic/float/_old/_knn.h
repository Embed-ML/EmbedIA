 ...
 * Uses a function pointer for flexible distance metric selection.
 */
typedef struct
{
    ...
    float *neighbors_features;      /**< Training data [n_samples × n_features] */
    uint16_t *neighbors_id;         /**< Class labels (classifier) or target values (regressor) */
    float (*distance_fn)(float*, float*, int);  /**< Distance function pointer */
