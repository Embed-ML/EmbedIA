 ...
 *
 * @details
 * - Uses 16-bit dfixed operations with extended range (4 additional bits)
 * - Taylor series approximations for transcendental functions
 * - Lookup tables for optimized sqrt() and trigonometric functions
 *
 * @note Default configuration uses FIX_FRC_SZ = 8, DFIXED uses FIX_DFRC_SZ = 6
...
/**
 * @brief Calculates hyperbolic tangent tanh(x) using lookup table.
 * @param x Fixed-point value.
 * @return tanh(x) with linear interpolation.
 */
fixed fixed_tanh(fixed x) {

    /* Paso 0.25 = 2^(-2), 13 intervalos + 1 sentinela para c=idx+2
     * Interpolación cuadrática Horner: 2 FIXED_MUL
    ...
    /* Horner grado 2: f(t) = a + t*(d1 + (t-1)*d2/2)
     * 2 FIXED_MUL en total                              */
    fixed inner = FIXED_MUL(t - FIX_ONE, d2 >> 1);  /* (t-1)*d2/2 */
    fixed result = a + FIXED_MUL(t, inner + d1);

    return (x >= 0) ? result : -result;
}

/**
 * @brief Calculates sine using Taylor series and symmetry.
