 ...
 *
 * QUANT8 OPTIMIZATION:
 *   Support vectors and coefficients are stored as storage_t (int8 quantized)
 *   to minimize flash usage. Intercepts are stored as compute_t (fixed16)
 *   for better precision. All kernel operations dequantize on-the-fly
 *   without additional RAM buffers.
 *
...
typedef struct {
    ...
    compute_t         gamma;     /* scale factor (poly, RBF, sigmoid)       */
    compute_t         intercept; /* bias term    (poly, sigmoid)            */
 ...
 *
 * QUANT8 STORAGE:
 *   vectors and coefs are stored as quantized int8 values to save flash.
 *   icepts are stored as compute_t (fixed16) directly for better precision.
 *   Each quantized array has its own qparam_t for dequantization.
 *
...
typedef struct {
    ...
    const compute_t     *icepts;     /* intercepts      [n_classes] (fixed16) */

    const qparam_t      qp_vectors; /* quantization params for vectors     */
    const qparam_t      qp_coefs;   /* quantization params for coefficients */
...
typedef struct {
    ...
    const qparam_t  qp_coefs;    /* quantization params for weights           */
 ...
 *
 * Support vectors and coefficients are dequantized on-the-fly from int8.
 * Intercepts are used directly as fixed16 values.
 *
 * @param[in]  model  Trained OvR model with quantized support vectors and coefs.
