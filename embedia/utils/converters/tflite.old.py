...
class OpCodeMapper:
    ...
    def _map_data(self, data):
        ...
        for idx, d in enumerate(data["operator_codes"]):
            self.code_to_name[idx] = self._builtin_code_to_name(d["builtin_code"])
            if self.code_to_name[idx] == "CUSTOM":
                self.code_to_name[idx] = self._name_list_to_str(d["custom_code"])
...
class TFLiteModelConverter:
    ...
    def set_from_buffer(self, flatbuffer, interpreter):
        ...
        self._structure = self._create_dict_from_flatbuffer(flatbuffer)
        self._model = None
        self._opcode_mapper = OpCodeMapper(self._structure)
    ...
    def _create_dict_from_flatbuffer(self, buffer_data):
    ...
    def _convert_model(self):
        ...
        for graph in self._structure["subgraphs"]:
            ...
            for operator in graph["operators"]:
                ...
                if name == 'FULLY_CONNECTED':
                    ...
                    layer = self.add_deepwise_conv2d(operator)
                elif name == 'RESHAPE':
    ...
    def add_input(self):
        sg = self._structure['subgraphs'][0]
    ...
    def add_conv2d(self, operator):
        (opt, inp) = (operator['builtin_options'], operator['inputs'])
    ...
    def add_deepwise_conv2d(self, operator):
        (opt, inp) = (operator['builtin_options'], operator['inputs'])
        bias = self._get_weights(inp[-1])
        weights_depth = np.transpose(self._get_weights(inp[-2]), (1, 2, 3, 0))
        weights_point = np.transpose(self._get_weights(inp[-2]+1), (1, 2, 3, 0))

        filters = weights_point.shape[3]
        kernel_size = (weights_depth.shape[0], weights_depth.shape[1])
        ...
        layer.set_weights([weights_depth, weights_point, bias])
